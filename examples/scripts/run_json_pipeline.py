...
If running on HPC:
- Set service URLs to point to running Apptainer instances or remote services
...
def run_pipeline_json_mode(
    ...
    if csv_folder and csv_folder.exists():
        ...
        try:
            ...
            logger.info("✓ Ingested %d posts in %.1f seconds", posts_ingested, dt)
        except Exception as e:
            logger.error("✗ Post ingestion failed: %s", e, exc_info=True)
            return 1
    else:
    ...
    if image_folders:
        ...
        try:
            ...
            dt = time.perf_counter() - t0
            logger.info("✓ Ingested %d images in %.1f seconds", images_ingested, dt)
        except Exception as e:
            logger.error("✗ Image ingestion failed: %s", e, exc_info=True)
            return 1
    else:
    ...
    if run_bio and images_ingested > 0:
        ...
        try:
            ...
            dt = time.perf_counter() - t0
            logger.info("✓ Analyzed %d images in %.1f seconds", analyzed, dt)
        except Exception as e:
    ...
    if run_bert and posts_ingested > 0:
        ...
        try:
            ...
            dt = time.perf_counter() - t0
            logger.info("✓ Analyzed %d posts in %.1f seconds", analyzed, dt)
        except Exception as e:
            logger.error("✗ Post sentiment analysis failed: %s", e, exc_info=True)
            # don't return, continue to other stages
    ...
    if run_qwen and images_ingested > 0:
        ...
        try:
            ...
            dt = time.perf_counter() - t0
            logger.info("✓ Qwen analyzed %d images in %.1f seconds", qwen_img, dt)
        except Exception as e:
            logger.error("✗ Qwen image analysis failed: %s", e, exc_info=True)
            # don't return, continue to other stages
    ...
    if run_qwen and posts_ingested > 0:
        ...
        try:
            ...
            dt = time.perf_counter() - t0
            logger.info("✓ Qwen analyzed %d comments in %.1f seconds", qwen_post, dt)
        except Exception as e:
            logger.error("✗ Qwen comment analysis failed: %s", e, exc_info=True)
            # don't return, continue to other stages
    ...
    try:
        ...
        logger.error("✗ Failed to write results: %s", e, exc_info=True)
        return 1
